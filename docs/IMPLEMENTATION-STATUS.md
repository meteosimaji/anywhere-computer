...
| Terminal | `sessions.py` owns persistent pipe processes, input, output cursors and cleanup. Opt-in POSIX PTY and Windows ConPTY paths include resize and control characters; real macOS PTY and Windows ConPTY integration tests pass in CI. | Installed Windows VM and same-Chat acceptance, plus broader interactive program coverage. Pipe remains the default. |
...
| GUI | Optional Peekaboo adapter. Historical Mac Calculator and TextEdit observations/actions/result recovery; Peekaboo 4 exact-window capture is required by the typed adapter; foreground fallback removed after a real cross-app input failure. Provider errors and native images now reach the MCP envelope; truncated and ambiguous observations cannot authorize input. | The current live TextEdit observation fails with an incomplete AX tree despite reported permissions; see [GUI reliability](GUI-RELIABILITY-2026-09-14.md). Default backend selection, redistribution/version policy, Windows GUI, focus/DPI/Japanese-input/stale-reference/disconnection suite. Earlier input mismatches remain relevant; individual successes do not qualify arbitrary apps. |
