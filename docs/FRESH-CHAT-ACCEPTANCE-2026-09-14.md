...
PR #31の変更を含むportableをMacへ適用し、HTTP常駐入口とCodex Pluginを更新した。
両入口と新規GPT-5.6 Sol中程度のChatで同じruntime
`fb28dae9a51d9a51d471bbaa122370f02119cfe067a525d21acba2c45393e508`、
instance `a7d0a857c5ab459786fe83e98e6a44e0` を確認した。版は0.1.0a9のままであり、
既存の公開alpha9配布物と同一内容という意味ではない。更新前の操作結果も台帳から回収した。

最初の新規ChatではGUI定義にwindow_idがなく、ChatGPT側に旧定義が残っていた。
設定の既存Pluginの「更新する」を実行すると、gui_observeの必須window_idと
gui_typeのelement_id・clearが表示された。再インストール・権限変更は行っていない。

さらに別の新規Chatを作成し、送信前にGPT-5.6 Sol選択済み・中程度を確認した。
そのChatでも新定義と同じエンジンを認識したが、指定したPeekaboo direct MCP
session開始がOpenAI側の自動安全性チェックで拒否されたと報告された。
セッションIDは発行されず、本文観測・入力・読戻しは未実施。別経路で再実行していない。
前の新規Chatでも試験ディレクトリ作成が同チェックで拒否され、ファイル試験は未実施だった。
具体的な拒否理由は返されていない。新定義の反映成功とGUIの実操作成功は区別する。

配布物経由の独立した機械GUI試験は [GUI記録](GUI-MCP.md) に記載。
その成功を今回のChatGPT GUI受け入れの代わりに扱わない。
