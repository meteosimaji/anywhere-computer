# Browser-free subchat recovery and later HTTP send
...
recovery strategy. The explicit-session path does not replace TLS identity or
forge a User-Agent. The optional Chrome-login path forwards that profile's
actual User-Agent with its Cookie for the authenticated HTTP GETs.
...
through recovery/wait. HTTPX queue dispatch is available only when a valid
generation handoff was explicitly supplied; the default HTTPX mode cannot send.
Neither adapter
