...
新しいパスワードは8文字以上・UTF-8で1024バイト以下とし、現在と同じ値は拒否します。
...
HTTP サーバーを停止します。共有ローカルエージェントを使う構成では、そのエージェントも
停止してください。両方の終了を確認してから、同じ状態ディレクトリで次を対話的に実行します。

```sh
anywhere owner-reset
```

保存済み HTTP 設定から所有者と接続先を読みます。引数で所有者や接続先を指定できません。
設定済み HTTP 端末の接続認可を失効させる確認として `REVOKE` を入力し、新しいパスワードを
非表示で２回入力します。確認が合わなければ認可とパスワードは変更されません。

HTTP サーバーまたは共有ローカルエージェントが実行中なら失敗します。停止すると
処理中の操作と未完了の同意画面も終了します。リセットでは設定済み端末を無効化して
その端末の既存 grant を失効させ、
その後 OS 資格情報ストアの検証データを更新・読み直します。保存結果が不明な場合も
端末は無効のままです。資格情報ストアを復旧したら `owner-reset` を再実行できます。
成功後も `http-enable` を明示的に実行し、クライアントで新規ログインしてください。
旧トークンは再有効化されません。
