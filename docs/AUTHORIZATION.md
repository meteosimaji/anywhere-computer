...
They do not yet provide a browser login, consent page, OAuth discovery endpoints,
token HTTP endpoint, or production service. There is no authentication bypass
...
binding, authorization-server and protected-resource metadata, the token endpoint
and its OAuth error contract, client registration/authentication policy, HTTPS,
